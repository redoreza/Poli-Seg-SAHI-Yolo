...
obj_detect_file_multi_bp = Blueprint('object_detection_file_multi', __name__, template_folder='../templates')
...
@obj_detect_file_multi_bp.route('/', methods=['GET'])
def file_multi_index():
    menus_active = mark_active_menu(request.path, menus)
    return render_template('object_detection_file_multi.html', menus=menus_active, title="Object Detection - Multiple File")

@obj_detect_file_multi_bp.route('/upload', methods=['POST'])
...
@obj_detect_file_multi_bp.route('/download-zip', methods=['POST'])
def file_multi_download_zip():
    ...
    memory_file.seek(0)
    return send_file(memory_file, mimetype='application/zip', as_attachment=True, download_name='detection_results.zip')

@obj_detect_file_multi_bp.route('/delete', methods=['POST'])
def file_multi_delete():
    files = request.json.get('files', [])
    if not files:
